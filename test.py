...
# Uncomment this for Python 3 and comment the below
#with open('po.csv', newline='') as csvfile:

#Uncomment this for Python 2 and Comment the previous one
with open('po.csv', 'r+') as csvfile:
...
s = requests.Session()
url = 'https://tools.usps.com/UspsToolsRestServices/rest/v2/appointmentTimeSearch'


# Month of April from 1-15 to put in the range
# for dt in range(1,15):
#     print(dt)
#     for locid in location:
#         payload = {"date":"202204"+str(dt),"productType":"PASSPORT","numberOfAdults":"0","numberOfMinors":"1","excludedConfirmationNumber":[""],"fdbId":[locid],"skipEndOfDayRecord":"true"}
...
for dt in range(int(sys.argv[2]),int(sys.argv[3])):
    print("Date : 2023" + str(sys.argv[1])+str(dt))
    for locid in location:
        print("Looking at location :" + getInfo(locid))
        # print("Location: ",locid)
        payload = {"date":"2023" + str(sys.argv[1])+str(dt),"productType":"PASSPORT","numberOfAdults":"0","numberOfMinors":"1","excludedConfirmationNumber":[""],"fdbId":[locid],"skipEndOfDayRecord":"true"}
